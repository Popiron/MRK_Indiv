...
namespace SimpleLang.Visitors
{
    public class GenCodeVisitor: Visitor
    {
        ...
        {
            var b = genc.DeclareLocal(typeof(int)); // переменная цикла cycle
            cond.expr.Visit(this); // сгенерировать команды, связанные с вычислением количества итераций цикла
            genc.Emit(OpCodes.Stloc, b);

            Label ifTrue = genc.DefineLabel();
            Label ifFalse = genc.DefineLabel();
            Label ifEnd = genc.DefineLabel();

            genc.Emit(OpCodes.Ldloc, b);
            genc.Emit(OpCodes.Ldc_I4_0);
            genc.Emit(OpCodes.Ble, ifFalse);// if i<=0 then goto endLoop
            genc.Emit(OpCodes.Ldloc, b);
            genc.Emit(OpCodes.Ldc_I4_0);
            genc.Emit(OpCodes.Bne_Un, ifTrue); // if i<=0 then goto endLoop
        ...
        {
            var i = genc.DeclareLocal(typeof(int)); // переменная цикла cycle
            rep.Expr.Visit(this); // сгенерировать команды, связанные с вычислением количества итераций цикла
            genc.Emit(OpCodes.Stloc, i); // i := кво итераций
